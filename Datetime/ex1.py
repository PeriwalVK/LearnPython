...
def _1_basic_date_and_time_objects():
    ...
    now = dt.datetime.now()
    specific_datetime = dt.datetime(2025, 12, 25, 14, 30, 45)
...
def _2_creating_datetime_objects():
    ...
    # From components
    dt1 = dt.datetime(2025, 10, 13, 23, 4, 30)
    print(f"From components: {dt1}")

    # Current date and time
    dt2 = dt.datetime.now()
    dt3 = dt.datetime.today()
    dt4 = dt.datetime.utcnow()  # UTC time

    print(f"dt.datetime.now(): {dt2}")
    print(f"dt.today(): {dt3}")
    print(f"dt.utcnow(): {dt4}")

    # From timestamp
    timestamp = 1697238270  # Unix timestamp
    dt5 = dt.datetime.fromtimestamp(timestamp)
    print(f"From timestamp: {dt5}")

    # From ISO format string
    iso_string = "2025-10-13T23:04:30"
    dt6 = dt.datetime.fromisoformat(iso_string)
    print(f"From ISO string: {dt6}")

    # Combining date and time objects
    my_date = dt.date(2025, 10, 13)
    my_time = dt.time(23, 4, 30)
    dt7 = dt.datetime.combine(my_date, my_time)
...
def _4_timedelta():
    ...
    td1 = dt.timedelta(days=7)
    td2 = dt.timedelta(hours=24)
    td3 = dt.timedelta(weeks=2, days=3, hours=4, minutes=30, seconds=45)
    td4 = dt.timedelta(milliseconds=500, microseconds=123)
    ...
    if days_ahead <= 0:  # Target day already happened this week
        days_ahead += 7
    next_friday = now + dt.timedelta(days=days_ahead)
...
def _5_timezone_handling():
    ...
    # Using dt.timezone.utc
    utc_now = dt.datetime.now(dt.timezone.utc)
    print(f"UTC: {utc_now}")

    # Using fixed offset
    est = dt.timezone(dt.timedelta(hours=-5))  # Eastern Standard Time
    est_now = dt.datetime.now(est)
    print(f"EST: {est_now}")
    ...
    try:
        ...
        ny_time = dt.datetime.now(ny_tz)
        tokyo_time = dt.datetime.now(tokyo_tz)
        london_time = dt.datetime.now(london_tz)
...
def _6_calender_operations():
    ...
    first_monday = first_of_month + dt.timedelta(days=days_until_monday)
    print(f"First Monday of the month: {first_monday}")

    # Last Friday of the month
    last_of_month = dt.date(
        today.year, today.month, calendar.monthrange(today.year, today.month)[1]
    )
    days_back_to_friday = (last_of_month.weekday() - 4) % 7
    last_friday = last_of_month - dt.timedelta(days=days_back_to_friday)
...
def _7_performance_and_best_practices():
    ...
    best_practices = [
        ...
        "Use dt.timedelta for time arithmetic instead of manual calculations",
...
def _8_common_datetime_patterns_and_recipes():
    ...
    end_of_week = start_of_week + dt.timedelta(
...
def _9_error_handling_and_edge_cases():
    ...
    # Invalid date
    try:
        invalid_date = dt.date(2025, 2, 30)  # February doesn't have 30 days
    except ValueError as e:
    ...
    # Parsing errors
    try:
        parsed = dt.datetime.strptime("invalid date", "%Y-%m-%d")
    except ValueError as e:
...
def _conclusion():
    ...
    print("• Working with dt.timedelta for time arithmetic")
